...
//     const bins: { from: number; to: number }[] = [];
//     for (let i = 0; i < k; i++) {
//       bins.push({ from: min + i * h, to: min + (i + 1) * h });
//     }
//     return bins;
//   };
...
//   const getHistogramData = (data: number[]) => {
//     const bins = getSturgesBins(data);
//     const freqs = bins.map(
//       (b) => data.filter((v) => v >= b.from && v < b.to).length
//     );
//     return {
//       labels: bins.map((b) => `${b.from.toFixed(2)}–${b.to.toFixed(2)}`),
//       datasets: [
...
//   const getStdDev = (data: number[]) => Math.sqrt(getVariance(data));
